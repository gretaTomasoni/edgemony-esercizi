...
// ! POST PER CREAZIONE CATEGORIA

const nameCategory = document.querySelector("#nameCategory");
const imageCategory = document.querySelector("#imageCategory");
const buttonCategory = document.querySelector("#postFormCategory");

buttonCategory.addEventListener("submit", (event) => {
  event.preventDefault();
  const objCategory = {
    name: nameCategory.value,
    image: imageCategory.value,
  };
  creationCategory(objCategory);
  console.log(objCategory);
});

const creationCategory = (objCategory) => {
  fetch("https://api.escuelajs.co/api/v1/categories", {
    ...
    .then((res) => res.json())
    .then((data) => console.log("RISPOSTA POST CATEGORIA: ", data))
    .catch((e) => console.log("ERRORE: ", e));
};
